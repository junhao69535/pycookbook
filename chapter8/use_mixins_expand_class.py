...
# 你有很多有用的方法，想使用它们来扩展其他类的功能。但是这些类并没有任何继承的关系。 因此你不能简单的将
# 这些方法放入一个基类，然后被其他类继承。
...
class StringKeyMappingMixin(object):
    ...
    def __setitem__(self, key, value):
        ...
        return super(StringKeyMappingMixin, self).__setitem__(key, value)


# 这些类单独使用起来没有任何意义，事实上如果你去实例化任何一个类，除了产生异常外没任何作用。
# 它们是用来通过多继承来和其他映射对象混入使用的。例如：
class LoggedDict(LoggedMappingMixin, dict):  # 继承顺序很重要，否则mixins类无效
    pass
...
d['x'].append(2)
d['x'].append(3)
# d['x'] = 23  # 会报错，KeyError错误


# 混入类在标准库中很多地方都出现过，通常都是用来像上面那样扩展某些类的功能。 它们也是多继承的一个主要用途。
# 比如，当你编写网络代码时候， 你会经常使用 socketserver 模块中的 ThreadingMixIn 来给其他网络相关类增加
# 多线程支持。 例如，下面是一个多线程的XML-RPC服务：
...
# 对于混入类，有几点需要记住。首先是，混入类不能直接被实例化使用。 其次，混入类没有自己的状态信息，也就是说
# 它们并没有定义 __init__() 方法，并且没有实例属性。 这也是为什么我们在上面明确定义了 __slots__ = () 。


# 还有一种实现混入类的方式就是使用类装饰器，如下所示：
def LoggedMapping(cls):
    """第二种方式：使用类装饰器"""
    cls_getitem = cls.__getitem__  # 这是获取继承得到的方法
