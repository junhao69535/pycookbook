...
# 想将内存映射为一个二进制文件到一个可变字节数组中，目的可能是为了随机访问它的内容或者
# 是原地做些修改。
import os
import mmap
...
# 可以给参数 access 赋值为 mmap.ACCESS_READ
# 如果你想在本地修改数据，但是又不想将修改写回到原始文件中，
# 可以使用 mmap.ACCESS_COPY
...
# 需要强调的一点是，内存映射一个文件并不会导致整个文件被读取到内存中。 也就是说，
